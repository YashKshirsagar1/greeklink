...
export default function Pledges() {
  ...
  async function handleAdd() {
    ...
    const { data } = await supabase.from('pledges').insert(row).select().single()
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-sm mx-4">
            ...
            <div className="mb-3">
              ...
              <input className="w-full bg-gray-800 text-white text-sm rounded-lg px-3 py-2.5 border border-gray-700 outline-none focus:border-yellow-400/50"
                placeholder="e.g. John Smith" value={newPledge.name} onChange={e => setNewPledge({ ...newPledge, name: e.target.value })} />
            </div>
            ...
            {selected ? (
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 sticky top-4">
                <div className="flex items-center gap-3 mb-4 pb-4 border-b border-gray-800">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold ${selected.color}`}>
                    {selected.initials}
                  </div>
                  <div>
                ...
                <div className="mt-4 flex justify-between items-center py-2 border-b border-gray-800">
                  ...
                  <button onClick={() => toggleDues(selected.id)}
                    className={`text-xs px-3 py-1 rounded-lg font-medium transition-all
                      ${selected.dues_paid ? 'bg-green-400/10 text-green-400 hover:bg-red-400/10 hover:text-red-400' : 'bg-red-400/10 text-red-400 hover:bg-green-400/10 hover:text-green-400'}`}>
                    ...
                    <div key={i} className="flex items-center gap-2 py-1.5">
                      <button onClick={() => toggleTask(selected.id, i)}
                        className={`w-5 h-5 rounded flex items-center justify-center text-xs flex-shrink-0 transition-all
                          ${selected.tasks[i] ? 'bg-green-400/20 text-green-400 border border-green-400/40' : 'bg-gray-800 text-gray-600 border border-gray-700'}`}>
                        {selected.tasks[i] ? '✓' : ''}
                      </button>
